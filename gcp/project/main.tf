...
resource "google_project_iam_custom_role" "api_enablement" {
...
resource "google_project_iam_member" "api_enablement" {
  count = var.enable_api_enablement ? 1 : 0

  project = var.project_id
  role    = google_project_iam_custom_role.api_enablement[0].id
  member  = "serviceAccount:${google_service_account.illumio.email}"
}
...
resource "google_project_iam_custom_role" "readwrite" {
...
resource "google_project_iam_member" "readwrite" {
  count = var.mode == "ReadWrite" ? 1 : 0

  project = var.project_id
  role    = google_project_iam_custom_role.readwrite[0].id
  member  = "serviceAccount:${google_service_account.illumio.email}"
}
...
resource "illumio-cloudsecure_gcp_project" "this" {
  ...
  depends_on = [
    ...
    google_project_iam_member.security_reviewer,
