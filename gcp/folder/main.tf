...
# -----------------------------------------------------------------------------
# Folder-Level Read-Only IAM Bindings
# -----------------------------------------------------------------------------
...
# Custom roles are created at the organization level because GCP does not
# support folder-level custom roles.
# -----------------------------------------------------------------------------
...
resource "google_folder_iam_member" "api_enablement" {
  count = var.enable_api_enablement ? 1 : 0
...
resource "google_folder_iam_member" "readwrite" {
  count = var.mode == "ReadWrite" ? 1 : 0
...
resource "illumio-cloudsecure_gcp_project" "this" {
  ...
  depends_on = [
    ...
    google_folder_iam_member.security_reviewer,
    google_folder_iam_member.compute_viewer,
    google_folder_iam_member.cloudasset_viewer,
